...
def evaluate(ast, environment, watch=None):
    ...
    if ast["tag"] == "call":
        ...
        val, status = evaluate(function["body"], local_environment, watch)
    ...
    if ast["tag"] == "assign":
        ...
        # Check if this is a new variable or a modification
        is_new = target_index not in target_base
        target_base[target_index] = value

    # Print watch notification if applicable
    if watch and target_index == watch:
        location = f"line {ast.get('line', '?')}"
        action = "Created" if is_new else "Modified"
        print(f"[Watching: {watch}] {action} at {location} with value: {value}")
